...
export default function TeamDetail() {
  ...
  const buildApiFilters = () => {
    ...
    // Team filter (always applied)
    apiFilters.team_id = team?.id;
  ...
  // Load insights for this team (with filters and pagination)
  ...
  // Initial load: fetch team first
  useEffect(() => {
    loadTeam();
  }, [teamId]);
  
  // Load insights and counts after team is loaded
  useEffect(() => {
    if (team) {
      loadCriticalityCounts();
      loadInsights();
    }
  }, [team, currentPage, filters]);
    ...
    return (
      <div className="p-8 max-w-[1600px] mx-auto w-full bg-background-light dark:bg-background-dark min-h-screen">
        <div className="text-center py-12">
          <p className="text-text-secondary dark:text-dark-secondary">Team not found</p>
          <button 
      ...
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center">
            <div className="w-16 h-16 bg-indigo-100 dark:bg-indigo-900/20 rounded-xl text-indigo-600 flex items-center justify-center mr-4">
            ...
            <div>
              ...
              <p className="text-text-secondary dark:text-dark-secondary text-sm">
                {team.department?.name || team.department || 'No Department'} • {team.members || 0} members
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-surface-light dark:bg-surface-dark rounded-card border border-border-light dark:border-border-dark overflow-hidden">
        ...
        {dataLoading ? (
          ...
          <div className="px-6 py-12 text-center text-text-secondary dark:text-dark-secondary">
            No insights found for this team.
          </div>
        ) : (
